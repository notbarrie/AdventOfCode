...
debugger;
debugger;
	directionHandler(arrTwo[i]);  
}
wireTwoVectors = currVector;
currVector = [];

console.log(wireOneVectors);
console.log(wireTwoVectors);

findIntersections(wireOneVectors, wireTwoVectors);
...
function calcClosest(){
  ...
  let currPoint = collisions[i];
  let distanceX = Math.abs(currPoint.x);
  let distanceY = Math.abs(currPoint.y)
  	if(distanceX+distanceY < shortest ){
    	shortest = Math.abs(collisions[i].x) + Math.abs(collisions[i].y);
...
function findIntersections(a1, a2){
  for(let j=0; j < a1.length; j++){
  	let currPosA1 = a1[j];
    for(let i=0; i < a2.length; i++){
    let currPosA2 = a2[i];
    	if(currPosA1.x === currPosA2.x && currPosA1.y === currPosA2.y){
      	collisions.push(currPosA1);
        index.push([i, j]);
      }
    }
  }
...
function directionHandler(command){
	//command syntax = R999 or D99 or U9 or L999
  //good
  let direction = command[0];
  let distance = +command.substring(1);
  
  if(direction === "U"){
  	// move positive distance in the y direction
    //distance, isPositive, y=1
    move(distance, true, false);
  } else if(direction=== "D"){
  	// move negative distance in the y direction
    //distance, isPositive, y=1
    move(distance, false, false);
  } else if(direction=== "L"){
  	// move negative distance in the x direction
    //distance, isPositive, y=1
    move(distance, false, true);
  } else if(direction=== "R"){
  	// move positive distance in the x direction
    move(distance, true, true);
  } 
}

function move(distance, isPositive, isX){
  	let point;
    if(isX){
    	if(isPositive){
    		//push {origin y, originx+distance} to movements 
        point = {x: origin.x + distance, y: origin.y };
        pushPoints(point, true);
      } else{
      	//push {origin y, originx+distance} to movements 
        point = {x: origin.x - distance, y: origin.y };
        pushPoints(point, false);
      }
 	 } else{
   if(isPositive){
    		//push {origin y, originx+distance} to movements 
        point = {x: origin.x, y: origin.y + distance};
        pushPoints(point, true);
      } else{
      	//push {origin y, originx+distance} to movements 
        point = {x: origin.x, y: origin.y - distance };
...
function pushPoints(point, isPositive){
...
if(isPositive){
    ...
    //get previous location from origin
//if x is the same, loop y
if(point.x === origin.x ){
	//loop from origin.y to point.y
  for(let i = origin.y; Math.abs(i) <= Math.abs(point.y); i += incrementer){
  	newPoint.y = newPoint.y + incrementer;
  	//add point to array;
    currVector.push({x: newPoint.x, y: newPoint.y});
  }
  	
  

} else if(point.y === origin.y){
	//loop though x values
  for(let i = origin.x; Math.abs(i) <= Math.abs(point.x); i += incrementer){

  	newPoint.x = newPoint.x + incrementer;
  	//add point to array;
    currVector.push({x: newPoint.x, y: newPoint.y});
  }
